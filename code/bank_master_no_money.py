...
class BankNoMoney():
    ...
    def _transaction(self, buyer_idx: int, seller_idx: int) -> None:
        # Buyer takes loan
        delta_debt = np.min((self.beta[buyer_idx] * self.p[buyer_idx] / self.interest_rate - self.d[buyer_idx], self.p[seller_idx]))
        delta_debt = np.max((delta_debt, 0))  # Ensure positive

        # Update values
        self.d[buyer_idx] += delta_debt  # buyer takes loan
        self.d_bank -= delta_debt  # bank gives loan
    ...
    def _bankruptcy(self) -> None:
        ...
        bankrupt_indices = np.where(self.p < self.interest_rate * self.d)[0]
        self.went_bankrupt = len(bankrupt_indices)
    ...
    def _update_free_interest(self, time_step: int) -> None:
        ...
        if delta_bank_debt < 0:
            self.interest_rate_free *= (1 - self.rho) 
        else:
            self.interest_rate_free *= (1 + self.rho / (1 - self.rho))
                
    
    def _adjust_interest_for_default_probability(self) -> None:
        # Calculate probability of default
        self.PD = np.min((self.went_bankrupt / self.N, 0.99))  # Prevent division by zero in next step
...
time_steps = 100
money_to_production_efficiency = 0.1  # Alpha
