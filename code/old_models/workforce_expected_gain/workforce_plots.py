...
class BankVisualization(general_functions.PlotMethods):
    def __init__(self, group_name, show_plots, add_parameter_text_to_plot):
        ...
        self.data_path = dir_path_output / "workforce_expected_gain_simulation_data.h5"
    ...
    def plot_companies(self, N_plot):
        ...
        ax1.set(ylabel="Number of machinery", title="Machinery")
        ax1.grid()
    ...
    def plot_number_of_bankruptcies(self):
        # Create figure
        fig, ax = plt.subplots()
        ax.plot(self.time_values, self.went_bankrupt / self.N, label="Bankruptcies")
    ...
    def plot_production_capacity(self):
        w_final = self.w[:, -1]
        m_final = self.m[:, -1]
        max_val = max(w_final.max(), m_final.max())
        bins = np.arange(0, max_val +1 , 1)
        
        fig, ax  = plt.subplots()
        counts, edges, _ = ax.hist(w_final, bins=bins, label=r"$w$", alpha=0.7)
        counts, edges, _ = ax.hist(m_final, bins=bins, label=r"$m$", alpha=0.7)
        ax.set(xlabel="Company workforce and machinery", ylabel="Counts", title="Production capacity distributions at final time")
        ax.grid()
