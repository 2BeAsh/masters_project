...
class BankVisualization(general_functions.PlotMethods):
    def __init__(self, group_name, show_plots, add_parameter_text_to_plot):
        ...
        self.data_path = dir_path_output / "workforce_quit_simulation_data.h5"
    ...
    def plot_production(self):
        ...
        fig, ax  = plt.subplots()
        counts, edges, _ = ax.hist(p_final, bins=bins)
        ax.set(xlabel="Number of employees", ylabel="Counts", title="Employee distribution at final time")
        ax.grid()
        # xticks
        number_of_xticks = 10
        xticks = np.linspace(0, p_final.max(), number_of_xticks, dtype=int)
