...
class Workforce():
    def __init__(self, number_of_companies, number_of_workers, salary_increase, time_steps):
        ...
        self.dir_path_image = Path.joinpath(self.dir_path, "images", "image_workforce_no_m_expected_gain")
    ...
    def _initialize_market_variables(self) -> None:    
        ...
        self.d = np.ones(self.N, dtype=float)       
    ...
    def _quit_job(self):
        ...
        for i in range(self.N):
            ...
            idx_other_comp = np.random.choice(idx, size=self.w[i]-1, replace=True)
    ...
    def _bankruptcy(self):
        ...
        bankrupt_idx = np.logical_or(self.w < self.interest_rate * self.d, self.w == 0)
        ...
        self.d[bankrupt_idx] = 1  # New company buys 1 machinery and gains debt equal to machinery price
        ...
        if idx_surving_companies.size != 0:  # There are non-bankrupt companies            
            ...
            self.salary = np.random.uniform(0.01, 1.1, number_of_companies_gone_bankrupt)
    ...
    def store_values(self) -> None:
        ...
        # File name and path
        file_name = "workforce_no_m_expected_gain_simulation_data.h5"
