...
class BankDebtDeflation():
    def __init__(self, number_of_companies: int, money_to_production_efficiency: float, interest_rate_change_size=0.01, beta_mutation_size=0.05, time_steps=1000):
    ...
    def _take_loan(self, buyer_idx, seller_idx):
        # Take loan trying to go positive
        delta_debt_smart = (self.beta[buyer_idx] * self.p[buyer_idx] + self.m[buyer_idx]) / self.interest_rate - self.d[buyer_idx]
        delta_debt_simple = self.p[seller_idx] - self.m[buyer_idx]
        delta_debt = np.min((delta_debt_smart, delta_debt_simple))  # Prevents the smart loan from taking a larger loan than necessary
        if delta_debt < 0:
            delta_debt = 0  # Only allow positive loans
            self.did_not_take_loan += 1
        # Update values
        self.m[buyer_idx] += delta_debt
        self.d[buyer_idx] += delta_debt 
        self.bank_money -= delta_debt
    ...
    def _set_interest_rate(self, time_step, N_update=1):
        ...
        if time_step >= 4:  # Ensure enough data points for derivative
            ...
            derivative = self._first_deriv_backwards(time_step)
            # derivative = self._second_deriv_backwards(time_step)
            self.derivative_list.append(1 * derivative)
        
        if len(self.derivative_list) == N_update:
    ...
    def _data_to_file(self) -> None:
        ...
        all_data = np.empty((self.N, self.time_steps, 7))
